...
Portal used for development/testing of this submission: portal ID `52016473` (visible in the account URL and in record URLs returned by the API, e.g. `.../record/0-1/{contactId}`; do not confuse with `hs_object_source_id`, which identifies the integration/app that wrote a record, not the portal itself).
...
- **Idempotent sync strategy**: `syncContactsWithHubSpot` uses HubSpot's native `POST /crm/v3/objects/contacts/batch/upsert` with `idProperty: 'email'` — email is unique for contacts by default, so HubSpot resolves create-vs-update atomically server-side against its primary object store, per record (see `contactRepository.upsertContactByEmail`). `syncDealsWithHubSpot` cannot use the same endpoint: deals have no default unique property, and `idProperty: 'dealname'` is rejected live with a `400` (`"Unable to perform update/upsert by non-unique 0-3 property dealname"`) — HubSpot's batch/upsert requires the id property to be genuinely unique. Deals therefore fall back to a client-side "search by dealname, then create/update" — see Known Limitations for the consistency trade-off this introduces. Both sync functions process records **sequentially** (not in parallel) to avoid bursting rate limits and to keep per-record error isolation simple: a single bad record is caught, logged, and recorded as `failed` in the returned summary without aborting the rest of the batch.
  - Docs: https://developers.hubspot.com/docs/api-reference/legacy/crm/objects/contacts/batch/upsert-contacts

## Known Limitations / Findings

- **HubSpot Search API eventual consistency (deals only)**: during live verification, running `syncDealsWithHubSpot` twice in immediate succession occasionally created a duplicate deal instead of updating, because the search index HubSpot uses for `findDealByName` had not yet reflected the deal created moments earlier by the first run. This is HubSpot-side indexing latency in the Search API, not a defect in the sync logic. **Contacts no longer have this exposure**: `syncContactsWithHubSpot` was migrated to `batch/upsert`, which resolves against HubSpot's primary object store instead of the eventually-consistent search index — confirmed live with two immediate back-to-back runs (`created: 3` then `updated: 3`, no duplicates). Deals still use search-then-write because deals have no unique property `batch/upsert` can key on (see Technical Decisions), so the same race remains possible there; running deal syncs back-to-back with a few seconds' gap avoids it, and a production system with tighter guarantees would need its own record of "which HubSpot ID does this local record map to" instead of relying on search-immediately-after-write.
- Deals sync doesn't use a bulk/batch HubSpot endpoint — each record is a separate PATCH/POST call, which is simpler to reason about at this scale (a handful of seed records) but would not scale efficiently to large datasets.
